...
export function useGameEngine() {
    ...
    const checkGameOver = (currentGrid, currentTray) => {
        const activeBlocks = currentTray.filter(b => b !== null);
    ...
    const processLines = (currentGrid) => {
    ...
    const canPlaceBlock = useCallback((block, row, col, currentGrid) => {
            ...
            for (let c = 0; c < block.shape[r].length; c++) {
                if (block.shape[r][c] === 1) {
                    ...
                    if (targetRow >= GRID_SIZE || targetCol >= GRID_SIZE || targetRow < 0 || targetCol < 0)
                        return false;
                    if (currentGrid[targetRow][targetCol] !== null && !currentGrid[targetRow][targetCol].endsWith('_clearing'))
                        return false;
                }
            }
        }
    ...
    const getValidPlacement = useCallback((block, row, col, currentGrid) => {
    ...
    const placeBlock = (trayIndex, rawRow, rawCol) => {
